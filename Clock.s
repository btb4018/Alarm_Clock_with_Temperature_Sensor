...
global	skip_byte, hex_24, hex_60
    
psect	udata_acs
clock_hrs: ds 1	;reserving byte to store clock hours time
clock_min: ds 1	;reserving byte to store clock minutes time
clock_sec: ds 1	;reserving byte to store clock seconds time

timer_start_value_1:	ds 1
timer_start_value_2:	ds 1
    
hex_60:	ds  1	;reserving byte to store decimal 60 in hex
hex_24:	ds  1	;reserving byte to store decimal 24 in hex
...
Clock_Setup: 
	...
	movlw	10000111B	; Set time0 to 16-bit, Fosc/4/256
...
Clock_Inc:	
	...
	movf	clock_sec, W, A	   
	cpfseq	hex_60, A	    ;check clock seconds is equal than 60
	...
	movf	clock_min, W, A
	cpfseq	hex_60, A	    ;check if minute time equal to 60
	return
	clrf	clock_min, A	    ;set minute time to 0 if = 60
	incf	clock_hrs, A	    ;increase hour time by one
	movf	clock_hrs, W, A	
	cpfseq	hex_24, A	    ;check if hour time equal to 24
	return	
